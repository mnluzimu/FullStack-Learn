...
def run_sft(
    ...
    # dataset needs +1 then cut back due to MCA shift logic
    data_args.cutoff_len += 1
    if data_args.is_openai_format:
        print("Processing OpenAI format dataset...")
        dataset_module = get_dataset_openai(template, model_args, data_args, training_args, stage="sft", **tokenizer_module)
    else:
